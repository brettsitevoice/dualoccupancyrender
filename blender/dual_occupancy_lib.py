"""
Shared geometry/build library — Dual Occupancy, 37 Osborne St Gerringong.

Used by both:
  build_dual_occupancy.py   (single static test-frame)
  build_animation.py        (stage-by-stage construction animation)
...
from stage_data import (  # noqa: E402
    LOT_WIDTH, LOT_DEPTH, RL_FRONT, RL_REAR,
    FRONT_SETBACK, BUILDING_WIDTH, BUILDING_DEPTH, GARAGE_WIDTH, GARAGE_DEPTH,
    WALL_THK, SLAB_THK, CLAD_THK, LINING_THK, FOOTING_DEPTH, ROOF_OVERHANG,
    RL_GARAGE_FFL, RL_GROUND_FFL, RL_FIRST_FFL, RL_RIDGE, ROOF_PITCH_DEG,
    bx0, bx1, by0, by1, gx0, gx1, gy0, gy1,
    ROOF_DROP, RL_EAVE_LOW, RL_FOOTING_LOW,
    STAGE_INFO, STAGE_BY_INDEX, STAGE_COLORS,
)
...
def get_material(name, color, alpha=1.0):
    ...
    if mat is None:
        ...
        # HASHED handles animated alpha (0 -> target) cleanly in EEVEE, for
        # every stage material, since all of them fade in from invisible.
...
def build_all_stages():
    """Builds stage_01 .. stage_16 collections and returns {index: collection}."""
    cols = {}

    # Stage 01 — Site strip / earthworks
    col = cols[1] = get_or_make_collection(STAGE_BY_INDEX[1]["key"])
    strip_pad_z = RL_FRONT - 0.30
    ...
    col = cols[2] = get_or_make_collection(STAGE_BY_INDEX[2]["key"])
    make_terrain("terrain_finished", col, 0, LOT_WIDTH, 0, LOT_DEPTH,
                 RL_FRONT, RL_REAR, mat_for_stage(2, "terrain"))

    # Stage 03 — Footings / foundations
    col = cols[3] = get_or_make_collection(STAGE_BY_INDEX[3]["key"])
    hollow_wall_box("footing_garage", col, gx0 - 0.15, gx1 + 0.15, gy0 - 0.15, gy1 + 0.15,
                     RL_FOOTING_LOW, RL_GARAGE_FFL, 0.30, mat_for_stage(3, "footing"))
    hollow_wall_box("footing_building", col, bx0 - 0.15, bx1 + 0.15, by0 - 0.15, by1 + 0.15,
                     RL_FOOTING_LOW, RL_GARAGE_FFL, 0.30, mat_for_stage(3, "footing"))

    # Stage 04 — Garage box slab (FFL 18.730)
    col = cols[4] = get_or_make_collection(STAGE_BY_INDEX[4]["key"])
    ...
    col = cols[5] = get_or_make_collection(STAGE_BY_INDEX[5]["key"])
    ...
    col = cols[6] = get_or_make_collection(STAGE_BY_INDEX[6]["key"])
    ...
    col = cols[7] = get_or_make_collection(STAGE_BY_INDEX[7]["key"])
    ...
    col = cols[8] = get_or_make_collection(STAGE_BY_INDEX[8]["key"])
    ...
    col = cols[9] = get_or_make_collection(STAGE_BY_INDEX[9]["key"])
    ...
    col = cols[10] = get_or_make_collection(STAGE_BY_INDEX[10]["key"])
    RAFTER_W = 0.070
    RAFTER_D = 0.240
    n_rafters = 9
    ...
    col = cols[11] = get_or_make_collection(STAGE_BY_INDEX[11]["key"])
    make_skillion_slab("roof_cladding", col,
                        bx0 - ROOF_OVERHANG, bx1 + ROOF_OVERHANG,
                        by0 - ROOF_OVERHANG, by1 + ROOF_OVERHANG,
                        RL_EAVE_LOW, RL_RIDGE, 0.060, mat_for_stage(11, "clad", alpha=0.55))

    # Stage 12 — External wall cladding
    col = cols[12] = get_or_make_collection(STAGE_BY_INDEX[12]["key"])
    mat_clad = mat_for_stage(12, "clad", alpha=0.55)
    ...
    col = cols[13] = get_or_make_collection(STAGE_BY_INDEX[13]["key"])
    mat_glaze = mat_for_stage(13, "glazing", alpha=0.5)
    door_w = 2.700
    ...
    col = cols[14] = get_or_make_collection(STAGE_BY_INDEX[14]["key"])
    ...
    col = cols[15] = get_or_make_collection(STAGE_BY_INDEX[15]["key"])
    ...
    col = cols[16] = get_or_make_collection(STAGE_BY_INDEX[16]["key"])
    mat_finish = mat_for_stage(16, "finish", alpha=0.45)
...
def compute_bbox(collections, indices):
    """World-space bounding box across the given stage indices' objects."""
